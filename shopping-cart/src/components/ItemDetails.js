...
const ItemDetails = ({match}) => {
    ...
    const addToCart = ()=>{
        const index = pokemons.findIndex((product) => product.id == match.params.id)
  ...
  return (
    ...
    <div className="pokemon-detail">
        ...
        <img className="poke-image-detail" src={item.sprites.front_default} alt="poke"/>
        <h4>Price: $50</h4>
    ...
    <div className="info-container">
        ...
        {item.abilities.map((item) => {
            return(
                <li>{capitalizeFirstLetter(item.ability.name)}</li>
            )
        }
        )}
        </ul>
