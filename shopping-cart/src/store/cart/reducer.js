...
export const cartReducer = createReducer({
  ...
  builder
    .addCase(addProductToCart, (state, action) => {
      if (state.products.findIndex((product) => product.pokemon.id === action.payload.id) === -1) {
        state.products.push({
          quantity: 1,
        ...
        state.products[state.products.findIndex((product) => product.pokemon.id === action.payload.id)].quantity++
      }
    })
    .addCase(increaseQuantity, (state, action)=>{
      state.products[state.products.findIndex((product) => product.pokemon.id === action.payload.pokemon.id)].quantity++
    })
